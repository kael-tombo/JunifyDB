...
| 53-WC-08 | Deployment | Site deploys from `kael-tombo.github.io/JunifyDB` while pushes go to `armand-ratombotiana/JunifyDB` | **RESOLVED — same repository** (2026-09-21): `kael-tombo/JunifyDB/branches/gh-pages` exposes commit `bbd87ca` pushed only to the armand-ratombotiana remote → the owner is a rename and GitHub redirects both paths. The earlier "frozen site / failing deploy" pattern is fully explained: Pages source is the `gh-pages` **branch** (`.nojekyll` present), so the Actions `deploy-pages` workflow was structurally incompatible — every `Deploy GitHub Pages` run fails, harmlessly. gh-pages had not been updated since Sep 17 | Medium | Site updates now flow: main `docs/index.html` → gh-pages publish (procedure documented in 53) |
