...
| R-28 | Console audit coverage | SQL Studio mutations bypassed the audit trail — `SqlHandler` never called the audit hook, so console-driven SQL writes left no `audit.log` entry | Low | **FIXED** (2026-09-21) | `/api/sql` now audits every mutation statement via `auditSqlStatements` (lexical split on semicolons outside quotes/comments; INSERT/UPDATE/DELETE/CREATE/DROP → same `logAuditEvent` ring+JSONL path; reads not audited; fail-open). `SqlAuditTrailTest` (3 tests) failed pre-fix (2 failures), passes post-fix; full gate green (692 tests, 73.31% line coverage); live-verified in the browser on JDK 25: SQL Studio INSERT → event in `/api/audit/logs` AND `audit.log` | | Fails in CI (runs #30, #31): starter modules never installed, so the 3 framework demos cannot resolve unpublished junify artifacts on fresh runners (local `.m2` cache masked this; job logs admin-only hid the cause) | Medium | **FIXED** (2026-09-21) | Public per-demo annotations diagnosed it (run #32: 6/9 pass, exactly the framework demos fail); local falsification: uninstall starter → reproduce, reinstall → green; fix = starter `mvn install` step before the demo loop (`1bc94a9`) |
...
- **Open**: R-13 (Maven Central staging — needs credentials), the mixed-writer half of R-20 (fundamental, documented in 13-T-03). R-28 (SQL Studio audit gap) was found in live preview validation and fixed the same day with regression tests. None are release blockers.
