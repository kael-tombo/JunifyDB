...
| R-16 | Error handling | No typed exception hierarchy | Medium | **FIXED** (2026-09-21) | `JunifyDBException` base + `StorageException`/`SerializationException`; 15 wrap sites migrated; 63-evidence |
...
| R-18 | Indexing | Planner never auto-selects secondary indexes | Medium | **FIXED** (2026-09-21) | Real defect: index walk used allValues() (no pruning). Now true point lookup via `idx.lookup(value)`; Query records eq value; 63-evidence |
| R-19 | Vectors | HNSW dimension hardcoded 128 | Medium | **FIXED** (2026-09-21) | Dimensionality derived from first client vector (or explicit `dims`); mismatch → clear 400; 63-evidence |
...
- **Fixed in improvement round 1 (2026-09-21)**: R-10, R-12, R-14, R-15, R-17, R-21, R-23, R-26 (CI gates, wrapper, CDC wiring, CORS, quarantine/atomic snapshots, WAL bound) — evidence in 62.
