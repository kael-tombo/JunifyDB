...
| ID | Item | Why | What | Risk | Deps | Acceptance & Verification |
|---|---|---|---|---|---|---|
| P1-1 | CI: coverage gate (R-10) | make the 70% claim real | measure current; set gate to measured value or fix CI to run `-Pcoverage-check` | low | P0-4 | CI fails on coverage drop |
| P1-2 | CI: starter/demo matrix (R-14) | upgrade integration claims to SUPPORTED | per-module jobs + demo smoke | low | none | matrix green badge in README |
| P1-3 | `mvnw` shell script (R-12) | Linux/macOS onboarding | add wrapper script | none | none | wrapper runs on bash |
| P1-4 | REST API reference | undocumented contracts (36-CB-01) | document endpoints/envelopes | none | none | every console call mapped |
| P1-5 | Playwright UI suite | visual/flow regression (58) | 10-flow script + screenshots | medium | none | CI artifact with screenshots |
| P1-6 | demo/README per-demo commands | remove hidden prerequisite (35-DM-01) | document `mvn install` + commands | none | none | fresh-clone demo run follows docs |

## P2 — 1.1
| ID | Item | Why | What |
|---|---|---|---|
| P2-1 | Maven Central publication (46-MC-01/02) | consumer adoption | source/javadoc/GPG plugins, staging, consumer-resolution proof |
| P2-2 | Typed exception hierarchy (21) | programmatic error handling | `JunifyDBException` tree + shims |
| P2-3 | CDC wiring behind flag or removal (23) | finish or remove | producer in doc write path, feature flag |
| P2-4 | CVE scan in CI (45-SC-02) | supply chain | OWASP dependency-check |
...
| P2-7 | Snapshot quarantine (18-F-02) | resilience | per-file health report + skip-corrupt policy flag |
