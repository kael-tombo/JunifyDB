...
| RB-24 | CI `demos` job red on every run where it executed (#30, #31) — blocks claiming "demos CI-verified"; local repro is 9/9 green so the product artifact is not in question | OPEN (mitigated) | Job reworked to emit public per-demo `::error::` annotations + surefire summaries; next run will reveal the CI-only root cause (R-27). If the next run is green, consider it environment-transient. |
